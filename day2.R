...
# Clean Input ----
cleaned_input <- strsplit(input, split = "\n")[[1]]
cleaned_input <- as.data.frame(cleaned_input)
names(cleaned_input) <- "input"

# Split each string by space into list. Then, bind inputs of each list into matrix. Then, convert matrix into dataframe
input_df <- data.frame(do.call("rbind", strsplit(cleaned_input$input, split = " ", fixed = TRUE)))
length_df <- data.frame(do.call("rbind", strsplit(input_df$X1, split = "-", fixed = TRUE)))
cleaned_df <- cbind(length_df, input_df[, c("X2", "X3")])

names(cleaned_df) <- c("len_x", "len_y", "alpha", "password")

cleaned_df$alpha <- sub(":", "", cleaned_df$alpha)
cleaned_df$len_x <- as.numeric(cleaned_df$len_x)
cleaned_df$len_y <- as.numeric(cleaned_df$len_y)


# Detect Password Policies ----
is_valid <- function(df) {
    x <- vector()
    for (i in 1:nrow(df)) {
        num_of_alpha <- sum(grepl(df$alpha[i], strsplit(df$password[i], split = "")[[1]]))
        
        if (num_of_alpha >= df$len_x[i] && num_of_alpha <= df$len_y[i]) {
            x <- c(x, TRUE)
        } else {
            x <- c(x, FALSE)
        }
...
sum(is_valid(cleaned_df))

# TROUBLESHOOT ----
# grepl(cleaned_df$alpha[55], cleaned_df$password[55])
# (num_of_alpha <- sum(grepl(cleaned_df$alpha[2], strsplit(cleaned_df$password[2], split = "")[[1]])))
# (num_of_alpha >= cleaned_df$len_x[2] && num_of_alpha <= cleaned_df$len_y[2])


# PART 2 ----
is_seq_valid <- function(df) {
    x <- vector()
    for (i in 1:nrow(df)) {
        seq_of_alpha <- grep(df$alpha[i], strsplit(df$password[i], split = "")[[1]])
        
        if (sum(seq_of_alpha %in% c(df$len_x[i], df$len_y[i])) == 1) {
            x <- c(x, TRUE)
        } else {
            x <- c(x, FALSE)
        }
